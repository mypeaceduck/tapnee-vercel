...
export async function POST(request: NextRequest) {
  try {
    ...
    const lastEntryResult = await query<{ lastTimestamp: string }>(
    ...
    if (
      ...
      timestamp.getTime() -
        new Date(lastEntryResult.rows[0].lastTimestamp).getTime() <
        THROTTLE_TIME
    ) {
    ...
    const improvementsResult = await query<Improvement>(
      "SELECT improvement FROM improvements WHERE gameId = $1 AND userId = $2 AND createdAt < NOW() - INTERVAL '1 day'",
      [gameId, userId]
    );
    const improvements = improvementsResult.rows.map((i) => i.improvement);

    const countSlapsResult = await query<{ countSlaps: number }>(
      "SELECT COUNT(*) as countSlaps FROM taps WHERE gameId = $1 AND userId = $2 AND areaId = $3 AND createdAt > NOW() - INTERVAL '1 minute'",
      [gameId, userId, areaId]
    );
    ...
    await query(
      ...
      [gameId, userId, areaId, timestamp.toISOString()]
    );
